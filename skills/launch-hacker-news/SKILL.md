...
description: |
  Prepare and run a Hacker News Show HN launch end to end with strict compliance: no vote mobilization,
  hype, marketing language, or emoji. Grounds the post in the real product, stack, and limitations, then
  writes a paste-ready POST.md, PLAN.md, CHECKLIST.md, and analytics plan under
  `.ulpi/launch/hacker-news/`. Covers the standard Show HN title, technical founder comment, readiness gate,
  US-Eastern timing, first-hours thread runbook, hard-question responses, and post-launch plan. Use when the
  user wants to prepare, write, or run a Show HN launch. Do not use to coordinate votes.
allowed-tools:
  - AskUserQuestion
  - Read
  - Write

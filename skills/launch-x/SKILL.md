...
name: launch-x
description: |
  Prepare and run a grounded product launch on X end to end. Writes a paste-ready POST.md, PLAN.md,
  CHECKLIST.md, OUTREACH.md, and analytics plan under `.ulpi/launch/x/`. Covers a scroll-stopping hook,
  launch thread, native media, the external product link in a reply, timing, golden-hour engagement,
  cross-promotion, readiness checks, and post-launch follow-up. Genuine audience and team engagement is
  allowed, while engagement bait, bought engagement, and mass DMs are prohibited. Use when the user wants
  an X or Twitter launch thread written or launch day planned and run.
allowed-tools:
  - AskUserQuestion
  - Read
  - Write

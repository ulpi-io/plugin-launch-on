...
description: |
  Prepare and run a grounded, policy-compliant Product Hunt launch end to end. Writes a complete LISTING.md,
  PLAN.md, OUTREACH.md, CHECKLIST.md, and analytics plan under `.ulpi/launch/product-hunt/`. Covers exact
  listing fields and limits, tagline options, description, topics, gallery shot list, demo outline, maker
  comment, T-minus plan, Pacific-time launch-day runbook, supporter outreach, readiness gate, and post-launch
  follow-up. Never supports vote manipulation, rings, bought or incentivized votes, or upvote requests. Use
  when the user wants a Product Hunt submission or full launch prepared and run.
allowed-tools:
  - AskUserQuestion
  - Read
  - Write
